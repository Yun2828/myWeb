...
import { activities, certifications, contactEmail, education, events, experience, githubRepositoriesUrl, linkedinUrl, openSourceContributions, presentation, projects, research, roleFocuses, skills, type Activity, type RoleFocus } from "@/data/portfolio";
...
function ActivityFeature({ item, index }: { item: Activity; index: number }) { const hasMedia = Boolean(item.image || item.images?.length); const compactPhoto = item.event === "AI Symposium 2026"; return <article className={`activity-feature-row ${compactPhoto ? "compact-event-photo" : ""} ${hasMedia ? (index % 2 === 0 ? "media-left" : "media-right") : "no-media"}`}>{hasMedia && <div className="activity-feature-media"><ActivityMedia item={item} /></div>}<div className="activity-feature-copy"><div className="activity-feature-heading">{item.date && <span className="date">{item.date}</span>}<p className="activity-feature-event">{item.event}</p><h3>{item.title ?? item.event}</h3>{!hasMedia && <ActivityLinks item={item} />}</div><div className="activity-feature-details">{item.description ? <p className="activity-feature-description">{item.description}</p> : <p className="activity-placeholder-copy">Details coming soon.</p>}<Highlights values={item.highlights} />{item.reflection && <p className="activity-feature-reflection">{item.reflection}</p>}<Tags values={item.technologies} />{hasMedia && <ActivityLinks item={item} />}</div></div></article>; }
function ResearchCard({ item, relevant }: { item: typeof research[number]; relevant: boolean }) {
  const [expanded, setExpanded] = useState(false);
  return <article className={`research-card accordion-research-card ${expanded ? "is-expanded" : ""} ${relevant ? "is-relevant" : ""}`}>
    <div className="research-card-inner">
      <div className="research-card-face research-card-front"><span className="date">{item.dates}</span><h3>{item.project}</h3><p className="research-role">{item.role} · {item.institution}</p><p className="research-summary">{item.summary}</p><p className="advisor">Advisor · {item.advisor}</p><Tags values={item.tags} /></div>
      <div className="research-card-face research-card-back" aria-hidden={!expanded}><div className="research-detail-groups">{item.detailGroups.map((group) => <div className="research-detail-group" key={group.label}><h4>{group.label}</h4><ul className="detail-list">{group.items.map((value) => <li key={value}>{value}</li>)}</ul></div>)}</div></div>
      <button className="research-detail-button" type="button" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}>{expanded ? "Close details" : "Learn more"} <span aria-hidden="true">{expanded ? "⌃" : "⌄"}</span></button>
    </div>
  </article>;
}
...
export function FocusPortfolio() {
  ...
  return <main>
    <header className="site-header shell"><a className="wordmark" href="#top" aria-label="Yun Waddy Oo home"><img className="brand-logo" src="/images/brand-logo.png" alt="Yun Waddy Oo logo" /><span className="brand-name">Yun Waddy Oo</span></a><nav aria-label="Primary navigation"><a href="#research">Research</a><a href="#work">Work</a><a href="#contact">Contact</a></nav></header>
    ...
    <section id="research" className="section shell intro-section"><div className="section-label">01 / Research</div><div className="research-grid">{visibleResearch.map((item) => <ResearchCard item={item} relevant={Boolean(focus && matches(item.roles, focus))} key={item.project} />)}</div></section>
    <section className="section shell presentation-section"><div className="section-label">02 / Presentation</div><div className="presentation-card"><p className="date">{presentation.status}</p><h2>{presentation.title}</h2><p className="lead">{presentation.venue} · {presentation.location}</p><p className="muted">{presentation.authors.join(" · ")} · {presentation.manuscript}</p></div></section>
    <section className="section shell skills-section"><div className="section-label">03 / Skills</div><div className="skills-grid">{visibleSkills.map((group, index) => <article className={`skill-card ${index % 2 === 0 ? "reveal-from-lower" : "reveal-from-higher"} ${focus && matches(group.roles, focus) ? "is-relevant" : ""}`} tabIndex={0} key={group.name}><span className="skill-card-number">0{index + 1}</span><h3>{group.name}</h3><div className="skill-card-list">{group.values.map((value) => <span key={value}>{value}</span>)}</div><span className="skill-card-arrow" aria-hidden="true">↗</span></article>)}</div></section>
    <section className="section shell experience-section"><div className="section-label">04 / Work experience</div>{visibleExperience.map((item) => <article className={`experience-item scroll-reveal ${focus && matches(item.roles, focus) ? "is-relevant" : ""}`} key={item.role}><div><span className="date">{item.dates}</span><h3>{item.role}</h3><p className="research-role">{item.organization}</p><Tags values={item.tags} /></div><div><p className="lead">{item.summary}</p><Highlights values={item.highlights} /></div></article>)}</section>
    <section id="work" className="section shell work-section" aria-live="polite"><div className="section-label">05 / Projects</div><FocusSelector active={focus} onChange={setFocus} /><div id="portfolio-results">{projectGroups.map(({ key, label, items }) => items.length ? <div className="project-category" key={key}><h3>{label}</h3><div className="project-grid">{items.map((project) => <article className={`project-card scroll-reveal ${focus && matches(project.roles, focus) ? "is-relevant" : ""}`} key={project.title}><div className="project-content"><h3>{project.title}</h3>{project.status && <span className="status">{project.status}</span>}<p>{project.description}</p><Highlights values={project.highlights} /><Tags values={project.technologies} />{project.github ? <a className="repository-link" href={project.github} aria-label={`View ${project.title} repository`} target="_blank" rel="noreferrer">View repository <Arrow /></a> : <span className="repository-placeholder">Repository link coming soon</span>}</div></article>)}</div></div> : null)}</div></section>
    <section className="section shell open-source-section"><div className="section-label">06 / Open source contribution</div><div className="open-source-list">{openSourceContributions.filter((item) => !focus || matches(item.roles, focus)).map((item) => <article className="open-source-card" key={item.title}><div><span className="date">Open source contribution</span><h3>{item.title}</h3><Tags values={item.technologies} /></div><div><p className="lead">{item.description}</p><a className="repository-link" href={item.github} aria-label={`View ${item.title} repository`} target="_blank" rel="noreferrer">View repository <Arrow /></a></div></article>)}</div></section>
