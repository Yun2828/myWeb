...
export function FocusSelector({ active, onChange }: { active: RoleFocus | null; onChange: (focus: RoleFocus | null) => void }) {
  ...
  const options: { value: RoleFocus | null; label: string }[] = [{ value: null, label: "View All" }, { value: "software-engineering", label: "Software Engineering" }, { value: "ai-ml", label: "AI / Machine Learning" }, { value: "retrieval-research", label: "Retrieval / Research" }];
  return <div className="project-focus-tabs" aria-labelledby="focus-heading"><div><p className="section-label" id="focus-heading">Explore</p><p className="focus-prompt">Browse my work</p></div><div className="focus-options" role="tablist" aria-label="Browse portfolio by focus">{options.map((option) => <button type="button" role="tab" key={option.label} className={active === option.value ? "focus-option active" : "focus-option"} aria-selected={active === option.value} aria-controls="portfolio-results" tabIndex={active === option.value ? 0 : -1} onClick={() => choose(option.value)}>{option.label}</button>)}</div></div>;
}
